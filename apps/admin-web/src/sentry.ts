...
const dsn = import.meta.env.VITE_SENTRY_DSN_ADMIN?.trim();
...
Sentry.setTag('app', 'admin-web');

export async function sendSentryVerificationError() {
  if (!dsn) return null;
  const eventId = Sentry.captureException(
    new Error('Sentry verification error: admin-web'),
    { tags: { app: 'admin-web', verification: 'true' } },
  );
...
if (import.meta.env.VITE_SENTRY_VERIFY_ON_STARTUP === 'true')
  void sendSentryVerificationError().then((eventId) => {
    if (eventId) console.info(`[sentry] admin-web verification event: ${eventId}`);
  });
